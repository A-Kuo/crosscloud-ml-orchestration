...
Architecture reference, coding conventions, and modification guidance for
contributors working on the CrossCloud ML Inference Router.
...
Input text
  └─► AttentionEntropyProbe (DistilBERT)
        └─► H_route = mean Shannon entropy across final N layers x all heads
...
| `tests/` | pytest suite — fast tests (mocked probes) + slow tests (`--runslow`, loads DistilBERT) |

## Key Mathematical Concepts

These are central to the codebase and must be preserved in any modifications:

- **Attention entropy:** `H(a) = -sum p(a_i) log p(a_i)` computed over softmax-normalised attention weights. High H = uniform attention = model uncertainty. Low H = focused attention = confidence.
- **H_route aggregation:** Mean entropy across all heads in the final N transformer layers. This is the scalar routing signal.
- **Threshold tau:** Not a fixed constant — calibrated nightly via isotonic regression on `(H_route, is_error)` pairs. Tau is the minimum H where `P(error|H) >= epsilon` (risk tolerance, default 0.15).
